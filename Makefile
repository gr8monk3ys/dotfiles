...
# Every target named here must exist, and every target not named here must be
# on the internal list in test_makefile.bats; that test fails otherwise.
help:
	@echo "Lorenzo's Dotfiles - Available Commands"
	@echo ""
	@echo "Install and link:"
	@echo "  make              - Full installation for the detected OS"
	@echo "  make macos        - macOS: Homebrew, casks, npm/Cargo, editor extensions, duti, link"
	@echo "  make arch         - Arch: pacman -Syu, the pacmanfile, link"
	@echo "  make link         - Link the checkout into \$$HOME (bin/link apply)"
	@echo "  make link-dry-run - Print what link would do, changing nothing"
	@echo "  make unlink       - Remove the links and restore a pre-link ~/.zshenv"
	@echo "  make restore-zshenv - Same as unlink"
	...
	@echo "                      the Firefox profiles that read it"
	@echo ""
	@echo "Packages (one manifest kind each):"
	@echo "  make brew-packages     - install/Brewfile"
	@echo "  make cask-apps         - install/Caskfile (day-one casks)"
	@echo "  make cask-apps-extra   - install/Caskfile.extra (optional; not in make macos)"
	@echo "  make node-packages     - install/npmfile"
	@echo "  make rust-packages     - install/Rustfile"
	@echo "  make vscode-extensions - install/Codefile (VSCodium, else VS Code)"
	@echo "  make pacman-packages   - install/pacmanfile (Arch)"
	@echo "  make duti              - macOS default apps from install/duti"
	@echo "  make brew-update       - brew update && brew upgrade"
	@echo "  make brew-cleanup      - brew cleanup"
	@echo ""
	@echo "Maintenance:"
	@echo "  make doctor       - Health check of the linked machine"
	@echo "  make update       - Fast-forward the checkout, update every kind, zinit, Neovim"
	@echo "  make backup       - Snapshot configs and installed packages"
	@echo "  make backup-compress - Snapshot, then compress it"
	@echo "  make backup-cleanup  - Keep only the five newest snapshots"
	@echo "  make restore [backup=/path] - Restore the latest (or a named) snapshot"
	@echo "  make clean        - Remove broken links into this checkout"
	@echo "  make bench-shell [runs=7] [budget=900] - Time zsh startup against a budget"
	@echo "  make worktree-add name=<task> [base=main] - New isolated worktree"
	@echo "  make worktree-list - List worktrees"
	@echo "  make worktree-remove name=<task> [force=1] - Remove a worktree"
	@echo "  make worktree-prune - Prune stale worktree metadata"
	@echo ""
	@echo "Checks:"
	@echo "  make lint         - shellcheck, markdownlint, stale refs, palette, doc links"
	@echo "                      (what CI's Lint job runs)"
	@echo "  make test-setup   - Install the suite's prerequisites (bats, zsh, stow, ...)"
	@echo "  make test         - Run the BATS suite"
	@echo "  make verify       - lint + test + both containers: the pre-push gate"
	@echo "  make daily        - Doc links + test suite, without the linters or Docker"
	@echo "  make verify-shellcheck | verify-markdown | verify-stale-refs | verify-palette"
	@echo "       | verify-doc-links | verify-tests | verify-docker - One part of verify"
	@echo "  make verify-config-live - Ask each installed tool whether it reads its config"
	@echo "  make test-docker  - Ubuntu container: make (link only), the suite, doctor"
	@echo "  make test-docker-arch - Arch container: make arch (whole pacmanfile), the suite, doctor"
	@echo "  make test-docker-interactive - A shell in the Ubuntu test image"
	@echo ""
	@echo "Automated sync (macOS):"
	@echo "  make sync-install   - Enable the daily fast-forward LaunchAgent"
	@echo "  make sync-uninstall - Disable it"
	@echo "  make sync-status    - Is it loaded; its recent log"
	@echo "  make sync-run       - Run bin/dotfiles-sync once, now"
	@echo ""
	@echo "Environment:"
	...
	@echo "  SKIP_LINTERS=1          - Skip shellcheck/markdownlint"
	...
	@echo ""
	@echo "OPERATING.md is the runbook."
