...
- `bin/link`: the write side of link state, behind `make link`/`link-dry-run`/`unlink`. `apply` and `dry-run` share one planner and print the same rows, so the preview can no longer drift from the real thing (the old `link-dry-run` was a hand-written mirror that omitted the runtime directory); every conflict is found before anything changes. It holds the only copy of the SSH Include line — `dotfiles-init` asks `link has-include` — and the list of **tool-owned** config dirs (`cava`, `mpd`), which are linked unfolded so a tool's own writes stay out of the checkout; a folded link from an older `make link` is converted in place. `test/test_link.bats` covers it against a fixture home
- `bin/validate-config-live` (and `make verify-config-live`): asserts that tools actually honour the configs this repo tracks. `make link` proves a file is in place and `bin/link-state` proves it resolves into the checkout; neither proves the tool reads it, and twelve configs here were tracked, linked, documented and applying to nothing. Each probe must be offline, fast, and fail when the config is _not_ honoured — asserting a file exists is not a probe, which is the mistake it exists to catch. Tools with no probe meeting that bar are deliberately absent rather than probed badly
...
- `.config/firefox/user.js` updated from arkenfox 140 to 144, the newest release, and installed into the live Firefox profile — it had never been applied to either profile. A fenced `[SECTION 9999]` override block keeps `privacy.clearOnShutdown_v2.cookiesAndStorage` false, because arkenfox's default logs you out of every site on each restart
...
- `make link` on Arch ran `pacman -Syu --noconfirm` (via `stow-arch: core-arch`) just to refresh symlinks; it now installs stow only if it is missing, and `make link-dry-run` installs nothing at all
- `make unlink` deleted `~/.zshenv` even when it was a real file rather than this checkout's link, and `make link` overwrote an existing `~/.zshenv.bak`; both now leave data they did not create alone
- `.config/cava/shaders/` and `themes/` (about 1,000 lines) were cava's first-run output, committed because the folded directory put them in the repo; removed, and cava is now linked unfolded
